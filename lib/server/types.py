...
class IPRange(object):
    ...
    def __init__(self, range_text):
        ...
        for sub_range in range_text.split('.'):
            if sub_range.count("-"):
                a, b = sub_range.split("-",1)
                start.append(a)
                end.append(b)
            else:
                start.append(sub_range)
                end.append(sub_range)
        self.start = IP('.'.join(start)).int()
        self.end = IP('.'.join(end)).int()
